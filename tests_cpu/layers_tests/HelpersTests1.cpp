...
TEST_F(HelpersTests1, evalHouseholderMatrix_test1) {
    ...
    double normX;
    NDArray<double> result = ops::helpers::evalHouseholderMatrix(x, normX);
...
TEST_F(HelpersTests1, biDiagonalizeUp_test1) {
    ...
    NDArray<double> pExp('c', {4,4}, {-17.1756,  24.3869, 0, 0, 0, -8.61985, -3.89823, 0, 0, 0,  4.03047, 4.13018,0,0,0, 1.21666});
    
    ops::helpers::biDiagonalizeUp(matrix);
    matrix.printBuffer();
    
    ASSERT_TRUE(matrix.isSameShapeStrict(&pExp));
    ASSERT_TRUE(matrix.equalsTo(&pExp));
}
